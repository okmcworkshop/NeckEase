...
| 步驟 | 連接點 A | 連接點 B | 說明 |
| :---: | :--- | :--- | :--- |
| 1 | 外部 5V 電源 | ESP32-C3 5V 輸入端 | 為開發板及馬達提供電源（馬達需經降壓） |
| 2 | ESP32-C3 3.3V | MPU-6050 VCC 腳位 | 提供 MPU-6050 感測器電源 |
| 3 | ESP32-C3 3.3V | 光敏電阻 一端 | 提供光敏電阻分壓電路上端電源 |
| 4 | ESP32-C3 GND | 所有零件 GND（馬達、感測器、電晶體） | 全部共地，形成完整迴路 |
| 5 | ESP32-C3 GPIO 1 | 1kΩ 電阻 一端 | 輸出震動馬達控制訊號 |
| 6 | 1kΩ 電阻 另一端 | 2N2222 三極管 基極（B 腳） | 限制基極電流，保護 GPIO |
| 7 | 2N2222 三極管 集極（C 腳） | 震動馬達 負極（黑線） | 開關管承接馬達負載電流 |
| 8 | 2N2222 三極管 射極（E 腳） | ESP32-C3 GND | 開關管導通時電流路徑接地 |
| 9 | 震動馬達 正極（紅線） | 3.3V（或經降壓之 5V） | 馬達供電正極 |
| 10 | 1N4007 陰極（有橫線端） | 震動馬達 正極 | 續流二極體反向並聯（陰極接正極） |
| 11 | 1N4007 陽極（無橫線端） | 震動馬達 負極 | 續流二極體反向並聯（陽極接負極），吸收反向感應電動勢 |
| 12 | 光敏電阻 另一端 | GPIO 3 及 10kΩ 電阻 一端 | 分壓電路中間節點（ADC 讀取點） |
| 13 | 10kΩ 電阻 另一端 | ESP32-C3 GND | 分壓電路下端接地 |
| 14 | MPU-6050 SDA 腳位 | ESP32-C3 GPIO 8 | I2C 資料線 |
| 15 | MPU-6050 SCL 腳位 | ESP32-C3 GPIO 9 | I2C 時脈線 |
| 16 | MPU-6050 GND 腳位 | ESP32-C3 GND | 感測器接地 |
